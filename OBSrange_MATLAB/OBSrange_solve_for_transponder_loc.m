% This script outputs a mat file with the best transponder location
% relative to the GPS.
%
% It operates like the main script but adds an extra loop over transponder
% location to solve for its position brute force. The best location
% has the minimum average RMS over all stations.
%
% Josh Russell, Zach Eilon, & Stephen Mosher 4/16/18
% JBR 2025 - modified to solve for transponder

clear; close all;

%% Parameters for grid search over transponder location

dforward_v = [-15:1:15]; %
dstarboard_v = [-10:1:10]; %
[dstarboard_gr,dforward_gr] = meshgrid(dstarboard_v,dforward_v);
...
%% Parameters
ifsave = 0; %1; % Save results to *.mat?
ifplot = 0; %1; % Plot results?

par = struct([]);
par(1).vp_w = 1500; % Assumed water velocity (m/s)
par.N_bs = 1; %1000; % Number of bootstrap iterations
...
% par.dforward = 0; %10; %5; % units of meters, positive means transponder is further forward than GPS 
% par.dstarboard = 0; %-5; %-5; % units of meters, positive means transponder is further starboard than GPS
...
par.rms_thresh = 3; %2.5; % Remove pings above this RMS residual threshold, calculated from a single travel-time inversion
...
rms_sum = zeros(size(dstarboard_gr));

icount=0;
Nstas_process = 0;
for ifw=1:length(dforward_v) % loop over forward
    for ist=1:length(dstarboard_v) % loop overstarboard
        
        icount = icount + 1;

        par.dforward = dforward_v(ifw); %10; %5; % units of meters, positive means transponder is further forward than GPS 
        par.dstarboard = dstarboard_v(ist); %-5; %-5; % units of meters, positive means transponder is further starboard than GPS
        ...
        for is = 1:Nstas
            ...
            if par.ifQC_ping
                ...
                while count<par.QC_iter
                    ...
                    data = dataQC;
                    N_badpings = N_badpings + length(data_bad(count+1).twt);
                    count = count + 1;
                end
    ...
    %             datamat.Ftest_res = Ftest_res;
            ...
            % Save total RMS
            rms_sum(ifw,ist) = rms_sum(ifw,ist) + mean(E_rms)*1000;
            
            Nstas_process = Nstas_process + 1;
        
        end % is % station
    end % ist % starboard
end % ifw % forward

rms_avg = rms_sum / Nstas_process;

%% Plot error surface

% Find indices of min value
[val, idx] = min(rms_avg(:)); % min over all elements
[irow, icol] = ind2sub(size(rms_avg), idx);

figure(999); clf; box on; hold on;
set(gcf,'color','w');
colormap(viridis);
colormap(brewermap(25,'Spectral'));
contourf(dstarboard_gr,dforward_gr,rms_avg,25,'linestyle','none');
% contour(dstarboard_gr,dforward_gr,rms_avg,10,'-k','linewidth',1.5);
plot(dstarboard_gr(irow,icol),dforward_gr(irow,icol),'ok','markerfacecolor',[1 1 1],'linewidth',1.5,'markersize',15);
cb = colorbar;
set(cb,'linewidth',1.5,'fontsize',15);
ylabel(cb,'Mean RMS (ms)','fontsize',15);
xlabel('Starboard (m)');
ylabel('Forward (m)');
title('Transponder position relative to GPS');
set(gca,'fontsize',15,'linewidth',1.5,'Layer','top');
axis equal

save2pdf([modified_outdir,'/plots/transponder_loc.pdf'],999,500)

% Save transponder coordinates
transponder.dstarboard_best = dstarboard_gr(irow,icol);
transponder.dforward_best = dforward_gr(irow,icol);
transponder.rms_avg_best = rms_avg(irow,icol);
transponder.rms_sum = rms_sum;
transponder.rms_avg = rms_avg;
transponder.Nstas_process = Nstas_process;
transponder.dstarboard_gr = dstarboard_gr;
transponder.dforward_gr = dforward_gr;
save([modified_outdir,'/mats/transponder_loc.mat'],'transponder');
